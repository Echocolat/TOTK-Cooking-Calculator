...
class TotKCookSim():
    ...
    def _monster_extract(self):
        ...
        if self._monster_extract_flag and not self._tmp['Recipe']['ResultActorName'] in ['Item_Cook_O_01', "Item_Cook_O_02"]:
            if effect != None and effect_time > 0:
                ...
                self._tmp['EffectTime'] = [60, 600, 1800]
            if (hitpoint_recover == 0 and effect != None) or effect == 'LifeMaxUp':
                ...
                self._tmp['EffectLevel'] = [self.effect[effect].get('MinLv'), effect_level + self.effect[effect].get('SuperSuccessAddVolume')]
                ...
                self._tmp['HitPointRecover'] = hitpoint_recover + self.effect['LifeRecover'].get('SuperSuccessAddVolume')
            elif effect != None:
                # if the meal is regenerative and has an effect (that is not hearty)
                self._monster_extract_health_level_random_flag = True
                self._tmp['HitPointRecover'] = [1, hitpoint_recover, hitpoint_recover + self.effect['LifeRecover'].get('SuperSuccessAddVolume')]
                self._tmp['EffectLevel'] = [self.effect[effect].get('MinLv'), effect_level, effect_level + self.effect[effect].get('SuperSuccessAddVolume')]
            else:
                # if the meal is regenerative with no effect
                self._monster_extract_only_health_random_flag = True
                self._tmp['HitPointRecover'] = [1, hitpoint_recover + self.effect['LifeRecover'].get('SuperSuccessAddVolume')]
    ...
    def _critical(self):
        ...
        if not self._monster_extract_flag and self._tmp['SuperSuccessRate'] > 0 and not self._tmp['Recipe']['ResultActorName'] in ['Item_Cook_O_01', "Item_Cook_O_02"]:
            ...
            if effect == None:
                ...
                self._tmp['HitPointRecover'] = [hitpoint_recover, hitpoint_recover + self.effect['LifeRecover'].get('SuperSuccessAddVolume')]
            elif effect == "LifeMaxUp":
                # level crit
                self._critical_only_level_flag = True
                self._tmp['EffectLevel'] = [effect_level, effect_level + self.effect[effect].get('SuperSuccessAddVolume')]
                ...
                if hitpoint_recover >= self.effect['LifeRecover'].get('MaxLv'):
                    ...
                    self._tmp['EffectTime'] = [effect_time, effect_time + self.system_data.get('SuperSuccessAddEffectiveTime')]
                else:
                    # health or time crit
        ...
        for material in materials_list:
            if material.get('CookTag') == "CookSpice":
                ...
                if self._critical_health_level_flag or self._critical_health_level_time_flag or self._critical_health_time_flag or self._critical_only_health_flag or self._monster_extract_only_health_random_flag:
                    hitpoint_recover = [hitpoint_recover[0] + material.get('SpiceBoostHitPointRecover', 0), hitpoint_recover[1] + material.get('SpiceBoostHitPointRecover', 0)]
                elif self._monster_extract_health_level_random_flag:
                    hitpoint_recover = [hitpoint_recover[0] + material.get('SpiceBoostHitPointRecover', 0), hitpoint_recover[1] + material.get('SpiceBoostHitPointRecover', 0), hitpoint_recover[2] + material.get('SpiceBoostHitPointRecover', 0)]
                else:
                ...
                if self._critical_only_time_flag or self._critical_health_time_flag or self._critical_health_level_time_flag:
                    effect_time = [effect_time[0] + material.get('SpiceBoostEffectiveTime', 0), effect_time[1] + material.get('SpiceBoostEffectiveTime', 0)]
                elif self._monster_extract_time_flag:
                    effect_time = [effect_time[0] + material.get('SpiceBoostEffectiveTime', 0), effect_time[1] + material.get('SpiceBoostEffectiveTime', 0), effect_time[2] + material.get('SpiceBoostEffectiveTime', 0)]
                else:
                    effect_time += material.get('SpiceBoostEffectiveTime')
    ...
    def _after_spice(self):
        ...
        if self._monster_extract_time_flag:
            self._tmp['EffectTime'] = [min(self._tmp['EffectTime'][0] + recipe.get('BonusTime', 0), 1800), min(self._tmp['EffectTime'][1] + recipe.get('BonusTime', 0), 1800), min(self._tmp['EffectTime'][2] + recipe.get('BonusTime', 0), 1800)]
        elif self._critical_only_time_flag or self._critical_health_time_flag or self._critical_health_level_time_flag:
            self._tmp['EffectTime'] = [min(self._tmp['EffectTime'][0] + recipe.get('BonusTime', 0), 1800), min(self._tmp['EffectTime'][1] + recipe.get('BonusTime', 0), 1800)]
        else:
            self._tmp['EffectTime'] = min(self._tmp['EffectTime'] + recipe.get('BonusTime', 0), 1800)
        # bonus health
        if self._critical_health_level_flag or self._critical_health_level_time_flag or self._critical_health_time_flag or self._critical_only_health_flag or self._monster_extract_only_health_random_flag:
            self._tmp['HitPointRecover'] = [min(120, self._tmp['HitPointRecover'][0] + recipe.get('BonusHeart', 0)), min(120, self._tmp['HitPointRecover'][1] + recipe.get('BonusHeart', 0))]
            self._tmp['HitPointRecover'] = [160 if self._tmp['HitPointRecover'][0] == 120 else self._tmp['HitPointRecover'][0], 160 if self._tmp['HitPointRecover'][1] == 120 else self._tmp['HitPointRecover'][1]]
            if effect == 'LifeMaxUp':
                self._tmp['HitPointRecover'] = [160, 160]
            if effect == None:
                self._tmp['HitPointRecover'] = [1 if self._tmp['HitPointRecover'][0] == 0 else self._tmp['HitPointRecover'][0], 1 if self._tmp['HitPointRecover'][1] == 0 else self._tmp['HitPointRecover'][1]]
        elif self._monster_extract_health_level_random_flag:
            self._tmp['HitPointRecover'] = [min(120, self._tmp['HitPointRecover'][0] + recipe.get('BonusHeart', 0)), min(120, self._tmp['HitPointRecover'][1] + recipe.get('BonusHeart', 0)), min(120, self._tmp['HitPointRecover'][2] + recipe.get('BonusHeart', 0))]
            self._tmp['HitPointRecover'] = [160 if self._tmp['HitPointRecover'][0] == 120 else self._tmp['HitPointRecover'][0], 160 if self._tmp['HitPointRecover'][1] == 120 else self._tmp['HitPointRecover'][1], 160 if self._tmp['HitPointRecover'][2] == 120 else self._tmp['HitPointRecover'][2]]
            if effect == 'LifeMaxUp':
                self._tmp['HitPointRecover'] = [160, 160, 160]
            if effect == None:
                self._tmp['HitPointRecover'] = [1 if self._tmp['HitPointRecover'][0] == 0 else self._tmp['HitPointRecover'][0], 1 if self._tmp['HitPointRecover'][1] == 0 else self._tmp['HitPointRecover'][1], 1 if self._tmp['HitPointRecover'][2] == 0 else self._tmp['HitPointRecover'][2]]
        else:
        ...
        if effect:
            if self._monster_extract_only_level_flag or self._monster_extract_health_level_random_flag:
                self._tmp['EffectLevel'] = [min(self.effect[effect].get('MaxLv'), self._tmp['EffectLevel'][0]), min(self.effect[effect].get('MaxLv'), self._tmp['EffectLevel'][1]), min(self.effect[effect].get('MaxLv'), self._tmp['EffectLevel'][2])]
                self._tmp['EffectLevel'] = [1.0 if self._tmp['EffectLevel'][0] <= 1.0 and self._tmp['EffectLevel'][0] > 0 else self._tmp['EffectLevel'][0], 1.0 if self._tmp['EffectLevel'][1] <= 1.0 and self._tmp['EffectLevel'][1] > 0 else self._tmp['EffectLevel'][1], 1.0 if self._tmp['EffectLevel'][2] <= 1.0 and self._tmp['EffectLevel'][2] > 0 else self._tmp['EffectLevel'][2]]
                if effect in ['LifeMaxUp', 'LifeRepair']:
                    self._tmp['EffectLevel'] = [4 * round(self._tmp['EffectLevel'][0] / 4), 4 * round(self._tmp['EffectLevel'][1] / 4), 4 * round(self._tmp['EffectLevel'][2] / 4)]
                    self._tmp['EffectLevel'] = [4 if self._tmp['EffectLevel'][0] <= 4.0 and self._tmp['EffectLevel'][0] > 0 else self._tmp['EffectLevel'][0], 4 if self._tmp['EffectLevel'][1] <= 4.0 and self._tmp['EffectLevel'][1] > 0 else self._tmp['EffectLevel'][1], 4 if self._tmp['EffectLevel'][2] <= 4.0 and self._tmp['EffectLevel'][2] > 0 else self._tmp['EffectLevel'][2]]
                self._tmp['EffectLevel'] = [math.floor(self._tmp['EffectLevel'][0]), math.floor(self._tmp['EffectLevel'][1]), math.floor(self._tmp['EffectLevel'][2])]
            elif self._critical_health_level_flag or self._critical_health_level_time_flag or self._critical_only_level_flag:
                self._tmp['EffectLevel'] = [min(self.effect[effect].get('MaxLv'), self._tmp['EffectLevel'][0]), min(self.effect[effect].get('MaxLv'), self._tmp['EffectLevel'][1])]
                self._tmp['EffectLevel'] = [1.0 if self._tmp['EffectLevel'][0] <= 1.0 and self._tmp['EffectLevel'][0] > 0 else self._tmp['EffectLevel'][0], 1.0 if self._tmp['EffectLevel'][1] <= 1.0 and self._tmp['EffectLevel'][1] > 0 else self._tmp['EffectLevel'][1]]
                if effect in ['LifeMaxUp', 'LifeRepair']:
                    self._tmp['EffectLevel'] = [4 * round(self._tmp['EffectLevel'][0] / 4), 4 * round(self._tmp['EffectLevel'][1] / 4)]
                    self._tmp['EffectLevel'] = [4 if self._tmp['EffectLevel'][0] <= 4.0 and self._tmp['EffectLevel'][0] > 0 else self._tmp['EffectLevel'][0], 4 if self._tmp['EffectLevel'][1] <= 4.0 and self._tmp['EffectLevel'][1] > 0 else self._tmp['EffectLevel'][1]]
                self._tmp['EffectLevel'] = [math.floor(self._tmp['EffectLevel'][0]), math.floor(self._tmp['EffectLevel'][1])]
            else:
    ...
    def _english(self):
        ...
        # effect time
        if self._monster_extract_time_flag:
            minutes0, seconds0 = divmod(self._tmp['EffectTime'][0], 60)
            minutes1, seconds1 = divmod(self._tmp['EffectTime'][1], 60)
            minutes2, seconds2 = divmod(self._tmp['EffectTime'][2], 60)
            ...
            effect_time_str = effect_time_str0 + " (33.3%), " + effect_time_str1 + " (33.3%), " + effect_time_str2 + " (33.3%)"
        elif self._critical_only_time_flag or self._critical_health_time_flag or self._critical_health_level_time_flag:
            minutes0, seconds0 = divmod(self._tmp['EffectTime'][0], 60)
            minutes1, seconds1 = divmod(self._tmp['EffectTime'][1], 60)
            effect_time_str0 = "{:02d}:{:02d}".format(int(minutes0), int(seconds0))
            ...
            if self._critical_only_time_flag:
                effect_time_str = effect_time_str0 + ", " + effect_time_str1 + " (100% if crit)"
            elif self._critical_health_time_flag:
                effect_time_str = effect_time_str0 + ", " + effect_time_str1 + " (50% if crit)"
            else:
                effect_time_str = effect_time_str0 + ", " + effect_time_str1 + " (33.3% if crit)"
        else:
        ...
        if self._monster_extract_only_health_random_flag or self._monster_extract_health_level_random_flag:
            whole_heart0, quarter_heart0 = divmod(self._tmp['HitPointRecover'][0], 4)
            whole_heart0, quarter_heart0 = int(whole_heart0), int(quarter_heart0)
            whole_heart1, quarter_heart1 = divmod(self._tmp['HitPointRecover'][1], 4)
            whole_heart1, quarter_heart1 = int(whole_heart1), int(quarter_heart1)
            whole_heart2, quarter_heart2 = divmod(self._tmp['HitPointRecover'][2], 4)
            whole_heart2, quarter_heart2 = int(whole_heart2), int(quarter_heart2)
            if effect == 'LifeMaxUp' or self._tmp['HitPointRecover'][0] == 160.0:
            ...
            if effect == 'LifeMaxUp' or self._tmp['HitPointRecover'][1] == 160.0:
            ...
            if effect == 'LifeMaxUp' or self._tmp['HitPointRecover'][2] == 160.0:
            ...
            if self._monster_extract_only_health_random_flag:
                ...
                heart_str = heart_str0 + " (50%), " + heart_str2 + " (50%)"
            elif self._monster_extract_health_level_random_flag:
                # 25% set to 1, 25% +3 hearts, 50% no change
                heart_str = heart_str0 + " (25%), " + heart_str1 + " (50%), " + heart_str2 + " (25%)"
            ...
            if self._critical_only_health_flag:
                # 100% +3 hearts
                heart_str = heart_str0 + ", " + heart_str1 + " (100% if crit)"
            elif self._critical_health_level_flag or self._critical_health_time_flag:
                # 50% +3 hearts
                heart_str = heart_str0 + ", " + heart_str1 + " (50% if crit)"
            else:
                # 33.3% +3 hearts
                heart_str = heart_str0 + ", " + heart_str1 + " (33.3% if crit)"
        else:
        ...
        if self._monster_extract_only_level_flag:
            level_str = str(self._tmp['EffectLevel'][0]) + " (50%), " + str(self._tmp['EffectLevel'][1]) + " (50%)"
        elif self._monster_extract_health_level_random_flag:
            level_str = str(self._tmp['EffectLevel'][0]) + " (25%), " + str(self._tmp['EffectLevel'][1]) + " (50%), " + str(self._tmp['EffectLevel'][2]) + " (25%)"
        elif self._critical_only_level_flag:
            level_str = str(self._tmp['EffectLevel'][0]) + ", " + str(self._tmp['EffectLevel'][1]) + " (100% if crit)"
        elif self._critical_health_level_flag:
            level_str = str(self._tmp['EffectLevel'][0]) + ", " + str(self._tmp['EffectLevel'][1]) + " (50% if crit)"
        elif self._critical_health_level_time_flag:
            level_str = str(self._tmp['EffectLevel'][0]) + ", " + str(self._tmp['EffectLevel'][1]) + " (33.3% if crit)"
        else:
        ...
        if effect:
            ...
            if self._monster_extract_only_level_flag or self._critical_only_level_flag or self._critical_health_level_flag or self._critical_health_level_time_flag:
                self._tmp['EffectLevel'] = self._tmp['EffectLevel'][0]
            elif self._monster_extract_health_level_random_flag:
                self._tmp['EffectLevel'] = self._tmp['EffectLevel'][1]
...
if __name__ == "__main__":
    meal = TotKCookSim()
    output = meal.cook(["Silver Boss Bokoblin Horn", "Star Fragment", "Hot-Footed Frog"])
    print(output)